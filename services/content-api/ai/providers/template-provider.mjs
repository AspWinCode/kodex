...
const TOPIC_SKELETONS = {
  if: {
    ...
    starter: 'def checkThreshold(n, limit):\n    pass\n',
  ...
  while: {
    ...
    starter: 'def firstNegative(lst):\n    pass\n',
    task: '[ЗАПОЛНИТЕ] Функция firstNegative(lst) возвращает номер первого отрицательного элемента или -1.',
    tests: [{ args: [[1, -2, 3]], expect: 1 }, { args: [[1, 2]], expect: -1 }],
  },
  loop: {
    fnName: 'sumAll',
    starter: 'def sumAll(lst):\n    pass\n',
    task: '[ЗАПОЛНИТЕ] Функция sumAll(lst) возвращает сумму всех чисел списка (0 для пустого списка).',
    tests: [{ args: [[1, 2, 3]], expect: 6 }, { args: [[]], expect: 0 }],
  },
  dict: {
    fnName: 'lookupValue',
    starter: 'def lookupValue(registry, key):\n    pass\n',
    task: '[ЗАПОЛНИТЕ] Функция lookupValue(registry, key) возвращает registry[key] или "НЕТ ДАННЫХ", если ключа нет.',
    tests: [{ args: [{ a: 1 }, 'a'], expect: 1 }, { args: [{}, 'z'], expect: 'НЕТ ДАННЫХ' }],
  },
  'multi-return': {
    fnName: 'minMax',
    starter: 'def minMax(lst):\n    pass\n',
    task: '[ЗАПОЛНИТЕ] Функция minMax(lst) возвращает [минимум, максимум] списка.',
    tests: [{ args: [[3, 1, 2]], expect: [1, 3] }],
  },
  generic: {
    fnName: 'echoValue',
    starter: 'def echoValue(x):\n    pass\n',
