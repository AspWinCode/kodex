...
Придумай ОДНО дело для заданной темы и верни СТРОГО валидный JSON (без markdown-обёртки, без пояснений вокруг), по такой схеме:
{
  ...
  "fnName": "имя функции на английском, camelCase — ДОЛЖНО быть побуквенно тем же именем, что в def внутри starter",
  "starter": "стартовый код на Python: комментарий(и) + определение функции (имя — то же самое значение, что в fnName выше, без исключений) с телом pass, ровно как учебная заготовка",
...
// Модель нередко называет fnName в JSON camelCase, а саму функцию в starter/
// hints пишет snake_case (или наоборот) — рассинхрон, из-за которого харнесс
// никогда не найдёт функцию по имени из fnName (см. реальный случай:
// fnName: "isTrustedCall", но starter содержал def is_trusted_call(...)).
// Поэтому fnName не берётся из ответа модели напрямую — он извлекается из
// первого def в самом starter, который и есть источник истины для харнесса.
function extractFnNameFromStarter(starter, fallback) {
  const match = typeof starter === 'string' && starter.match(/^\s*def\s+([A-Za-z_][A-Za-z0-9_]*)\s*\(/m);
...
export async function generate({ topic, existingIds = [] }) {
  ...
  return {
    ...
    fnName: extractFnNameFromStarter(draft.starter, draft.fnName || 'solve'),
